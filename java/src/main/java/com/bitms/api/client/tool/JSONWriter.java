...
import com.bitms.api.client.mapping.ApiListField;
...
public class JSONWriter
{
    ...
    private void value(Object object, boolean isApiModel)
    {
        ...
        else
        {
            ...
            else if (object.getClass().isArray()) array(object, isApiModel);
            else if (object instanceof Iterator<?>) array((Iterator<?>) object, isApiModel);
            else if (object instanceof Collection<?>) array(((Collection<?>) object).iterator(), isApiModel);
            else if (object instanceof Date) date((Date) object);
            else
            {
                if (isApiModel) model(object);
                else bean(object);
            }
            calls.pop();
        }
    ...
    private void model(Object object)
    {
        ...
        {
            BitmsLogger.logBizError(e1);
        }
        catch (IllegalAccessException e2)
        {
            BitmsLogger.logBizError(e2);
        }
        catch (IllegalArgumentException e3)
        {
            BitmsLogger.logBizError(e3);
        }
        catch (InvocationTargetException e4)
        {
            BitmsLogger.logBizError(e4);
        }
        add("}");
    }
